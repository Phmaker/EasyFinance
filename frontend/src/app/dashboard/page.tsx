...
import { FiMenu, FiTrendingUp, FiTrendingDown, FiDollarSign, FiHash, FiActivity, FiTag, FiChevronDown, FiPlusCircle } from 'react-icons/fi';
...
const BudgetGoals = ({ goals, onAddClick }: { goals: BudgetGoal[]; onAddClick: () => void; }) => {
    ...
    return (
        <div className="rounded-xl border border-black bg-black p-6">
            <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={onAddClick} className="text-blue-400 hover:text-blue-300 transition-colors" title="Adicionar nova meta"><FiPlusCircle/></button>
            </div>
            <div className="space-y-4">
                {goals?.length > 0 ? goals.map(goal => (
                    <div key={goal.id}>
                        <div className="flex justify-between text-sm mb-1">
                            <span className="text-slate-300">{goal.name} {goal.category_name && `(${goal.category_name})`}</span>
                            <span className={`font-medium ${goal.current_amount > goal.target_amount ? 'text-red-400' : 'text-white'}`}>
                                {formatCurrency(goal.current_amount)} / {formatCurrency(goal.target_amount)}
                            </span>
                        </div>
                        <div className="w-full bg-slate-700 rounded-full h-2.5">
                            <div className={`h-2.5 rounded-full ${goal.current_amount > goal.target_amount ? 'bg-red-500' : 'bg-blue-600'}`} style={{width: `${Math.min((goal.current_amount / goal.target_amount) * 100, 100)}%`}}></div>
                        </div>
                    </div>
                )) : (
...
export default function DashboardPage() {
  ...
  // --- CORREÇÃO: Usando os nomes padrão isLoading e error ---
  ...
  const handleSaveGoal = async (goalData: BudgetGoalPayload) => {
    try {
        await api.post('/budget-goals/', goalData);
        mutateGoals(); // Revalida os dados das metas para atualizar a lista
  ...
  const MainContent = () => (
    <div className="space-y-6">
      ...
      <div>
        <BudgetGoals goals={goalsData || []} onAddClick={() => setGoalModalOpen(true)} />
      </div>
    </div>
  );
    ...
    <AnimatedLayout>
      <div className="flex h-screen overflow-hidden">
        ...
        <main className="flex-1 p-4 md:p-8 overflow-y-auto">
          ...
          {/* --- 5. CORREÇÃO: Usando as variáveis corretas para loading e error --- */}
        ...
        <AddGoalModal 
            ...
            categories={categoriesData?.results?.filter(c => c.type === 'expense')}
        />
      </div>
    </AnimatedLayout>
  );
